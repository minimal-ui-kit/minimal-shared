...
type Props = {
  ...
  options: {
    ...
    colors: { value: SettingsState['navColor']; icon: string }[];
  };
  onClickOption: {
...
export function NavOptions({ options, value, onClickOption, hideNavColor, hideNavLayout }: Props) {
  ...
  const renderLayout = !hideNavLayout && (
  ...
  const renderColor = !hideNavColor && (
    <Box gap={1.5} display="flex" flexDirection="column">
      <Box component="span" sx={labelStyles}>
        Color
      </Box>
      <Box gap={1.5} display="flex">
        {options.colors.map((option) => (
          <ColorOption
            ...
            onClick={() => onClickOption.color(option.value)}
          />
        ))}
      </Box>
    </Box>
...
type ColorOptionProps = ButtonBaseProps & {
  selected: boolean;
  option: { value: SettingsState['navColor']; icon: string };
};

export function ColorOption({ option, selected, sx, ...other }: ColorOptionProps) {
  return (
    <BlockOption
      selected={selected}
      icon={<SvgColor src={`'${option.icon}'`} />}
      label={option.value}
      sx={{
        gap: 1.5,
        height: 56,
